...
module.exports = function() {
	...
	function onControllerListener(e, status, response) {
		...
		switch(eventType) {
			case ORDER_EVENT.ORDERS_INFO:
				...
				$('.js-product-count').text(productsInfo.length);
				$('.js-basePrice').text(util.currencyFormat(data.paymentInfo.totalPaymentPrice-totalDeliveryCharge));
				$('.js-basicDiscount').text(data.paymentInfo.totalDiscountPriceDesc);
				$('.js-usedPoint').text('0');
				$('.js-handlingPrice').text(util.currencyFormat(totalDeliveryCharge));
				$('.js-totalPrice').text(data.paymentInfo.totalPaymentPriceDesc);
	...
	function reCalculatePointUse() {
		...
		$('.js-usedPoint').text(util.currencyFormat(usePoint));
		$('.js-totalPrice').text(util.currencyFormat(baseTotalPrice-usePoint));		
