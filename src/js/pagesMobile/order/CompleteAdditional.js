...
module.exports = function() {
	...
	fileName = 'order/CompleteAdditional.js';
	...
	function onControllerListener(e, status, response, elements) {
		...
		switch(eventType) {
			default:
				...
				result.data.paymentInfo = result.data.paymentMethods[0];
				result.data.paymentInfo.paymentPriceDesc = util.currencyFormat(result.data.paymentInfo.paymentPrice);
	...
	function myInfoResultHandler(e) {
		...
		if (loginData == null) {
			...
			controller.ordersAdditionalComplete(self.orderNumber);
		}
	}
