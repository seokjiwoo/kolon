...
module.exports = function() {
	...
	function opinionsClassHandler(e, status, result) {
		...
		$('#alignDrop').on('change', function() {
			controller.opinionsList($(this).val(), currentPage, 2);
		});
		
		controller.opinionsList(listOrder, currentPage, 2);
	}
	...
	function opinionsListHandler(e, status, result) {
		if (status == 200) {
			...
			$('.pageButton').click(function(e){
				e.preventDefault();
				$('#opinionList').empty();
				controller.opinionsList(listOrder, $(this).data().targetPage, 2);
			});
			...
			$.map(opinions, function(each) {
				...
				if (each.answerCount == 0) {
					each.answerCountTag = '<b>미답변</b><a href="#" id="writeCommentButton' +  each.opinionNumber + '" class="fl_r icoComment writeCommentButton">&nbsp;<a/>';
				} else {
					// each.answerCountTag = '<b class="pointCyan">답변완료</b><span class="pointBrown">'+each.answerCount+'개</span>';
					each.answerCountTag = '<b class="pointCyan">답변완료</b><a href="#" id="writeCommentButton' +  each.opinionNumber + '" class="fl_r icoComment writeCommentButton">';
					each.answerCountTag += '<span class="pointBrown">' + each.answerCount + '</span></a>';
				}
	...
	function showWriteForm(e) {
		...
		if (loginData != null) {
			if (!$(this).hasClass("active")) {
				...
				$('html, body').delay(200).animate({scrollTop: $(this).offset().top});
			} else {
	...
	function onUploadSuccess(e, result) {
		...
		if (uploadImageArray.length == 3) {
			...
			$('#fileUpList').append('<div id="con'+uploadFileNumber+'" class="conDel except except02">'+result.attachFileName+' <a href="#" id="deleteFile'+uploadFileNumber+'" data-image-url="'+result.attachFileUrl+'" class="btnDel">삭제</a></div>');
